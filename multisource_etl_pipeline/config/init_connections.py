...
def initialize_all_connections():
    ...
    # 1. Загружаем .env файл если есть (переменная AIRFLOW_ENV_FILE)
    env_file = os.getenv('AIRFLOW_ENV_FILE', '.env')
    if os.path.exists(env_file):
        logger.info(f"Загружаем переменные из {env_file}")
        load_env_file(env_file)
    
    # 2. Определяем конфигурацию connections
    connections_config = {
        ...
        'source_mongo': {
            'type': 'mongo',
            'env_prefix': 'SOURCE_MONGO'
        },

        # FTP connections
        'source_ftp': {
            'type': 'ftp',
            'env_prefix': 'SOURCE_FTP'
        },
        
        # API connections
        'source_api': {
            'type': 'http',
            'env_prefix': 'SOURCE_API'
        },
    }
    
    # 3. Инициализируем connections из ENV
    logger.info(f"\nИнициализация {len(connections_config)} connections...")
    init_connections_from_config(connections_config)
    
    # 4. Создаем дополнительные connections напрямую (если нужно)
    logger.info("\nСоздание дополнительных connections...")
    
    # Пример: локальное соединение для разработки
    if os.getenv('AIRFLOW_ENV', 'prod') == 'dev':
        create_or_update_connection(
    ...
    # 5. Выводим список созданных connections
    logger.info("\n" + "-" * 40)
    logger.info("СОЗДАННЫЕ CONNECTIONS:")
    logger.info("-" * 40)
    
    # 6. Вывод всех созданных Connections
