...
## Архитектура и SCD Type 2

**Потоки данных:** PostgreSQL (заказы, клиенты, order_items) + MongoDB (отзывы) + CSV (продукты) + FTP (логи доставки) + API (опционально) → **Extract** → **Transform** (валидация, нормализация) → **Load** в аналитическую БД (`daily_business_analytics`) и в DWH (схема «звезда»).

**SCD Type 2:** Измерения `dim_customers` и `dim_products` хранят историю версий: при изменении атрибута старая версия закрывается (`expiration_date`, `is_current = FALSE`), создаётся новая версия. При загрузке фактов `fact_orders` используется `get_dimension_key_for_date()` — привязка к той версии измерения, которая была актуальна на **дату заказа**, что обеспечивает корректность исторических отчётов.

**Расписание:** DAG `main_etl` запускается ежедневно в **9:00** (`schedule='0 9 * * *'`). Зависимости: параллельное извлечение из всех источников → валидация объёмов → трансформация → параллельная загрузка в Analytics и DWH.
